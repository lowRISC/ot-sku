...
extern const unsigned char open_owner[2048];
...
rom_error_t sku_creator_owner_init(boot_data_t *bootdata) {
  const owner_keydata_t *owner = &((const owner_block_t *)open_owner)->owner_key;
  uint32_t config_version = ((const owner_block_t *)open_owner)->config_version;
  ...
  memcpy(&owner_page[0], open_owner, sizeof(open_owner));
